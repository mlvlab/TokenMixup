...
class ViTLite(nn.Module):
    ...
    def forward(self, x, y=None, grad_sc=None, analysis=0):
        x = self.tokenizer(x)  # torch.Size(128(B), 256(W), 256(H))
        return self.classifier(x, y, grad_sc)
...
def _vit_lite(arch, pretrained, progress,
    ...
    if pretrained and arch in model_urls:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        if kwargs['positional_embedding'] == 'learnable':
            state_dict = pe_check(model, state_dict)
        elif kwargs['positional_embedding'] == 'sine':
